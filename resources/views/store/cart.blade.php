            ...
            <div class="overflow-hidden rounded-sm border border-line-200 bg-white">
                ...
                @foreach ($items as $item)
                    ...
                    <article class="grid grid-cols-[88px_1fr] gap-4 p-4 sm:grid-cols-[110px_1fr_auto] sm:gap-5 sm:p-5">
                        <a href="{{ route('store.product', [$product->category->section->slug, $product->slug]) }}" class="aspect-[4/5] overflow-hidden rounded-sm bg-[#efebe5]">@if ($cartImage)<img src="{{ $cartImage->displayUrl() }}" alt="{{ $product->name_ar }}" class="h-full w-full object-cover">@else<div class="flex h-full items-center justify-center p-2 text-center text-[9px] text-stone-400">ركاز × لجين</div>@endif</a>
